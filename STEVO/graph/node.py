...
class Node(ABC):
    ...
    def _add_tokens(self, added_prompt_token: int, added_completion_token: int) -> int:
        self.prompt_tokens += added_prompt_token
        self.completion_tokens += added_completion_token

    def _add_logprob(self, logprobs: list[dict]) -> None:
        for item in logprobs:
            lst = []
            toplogprobs = item['top_logprobs']
            for logprob in toplogprobs:
                lst.append(logprob['logprob'])
            entropy = -sum([math.exp(p) * p for p in lst])
