...
def parse_args():
    parser = argparse.ArgumentParser(description="STEVO Experiments on aqua")
    parser.add_argument("--train_json", type=str, default="datasets/aqua/train.json")
    parser.add_argument("--test_json", type=str, default="datasets/aqua/test.json")
...
async def main():
    ...
    with open(args.train_json, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if line:  # 跳过空行
                train_data.append(json.loads(line))  # 逐行解析

    train_data = aqua_data_process(train_data)
    ...
    with open(args.test_json, "r", encoding="utf-8") as f:
        for line in f:
            line = line.strip()
            if line:  # 跳过空行
                test_data.append(json.loads(line))  # 逐行解析

    test_data = aqua_data_process(test_data)

    current_time = Time.instance().value or time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
    Time.instance().value = current_time
    result_dir = Path(f"{STEVO_ROOT}/result/aqua")
