...
from datasets.multiarith_dataset import multiarith_data_process, multiarith_get_predict
...
def parse_args():
    parser = argparse.ArgumentParser(description="STEVO Experiments on gsm8k")
    parser.add_argument("--train_json", type=str, default="datasets/multiarith/train.json")
    parser.add_argument("--test_json", type=str, default="datasets/multiarith/test.json")
    ...
    parser.add_argument('--decision_method', type=str, default='FinalRefer',
                        help='The decison method of the STEVO')
...
async def main():
    ...
    train_data = multiarith_data_process(train_data)

    with open(args.test_json, "r", encoding="utf-8") as f:
        test_data = json.load(f)
    test_data = multiarith_data_process(test_data)

    current_time = Time.instance().value or time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())
    Time.instance().value = current_time
    result_dir = Path(f"{STEVO_ROOT}/result/multiarith")
    ...
    for i_batch in range(num_batches):
        ...
        current_batch = dataloader(train_data, args.batch_size, i_batch)
        ...
        for task, answer, log_prob, regular_loss, potential_traj, true_answer in zip(current_batch, raw_answers,
                                                                                     log_probs, regular_loss_lst,
                                                                                     potential_trajs, answers):
            predict_answer = multiarith_get_predict(answer[0])
            ...
            if utility and graph.evolver.training:
    ...
    num_batches = int(len(test_data) / args.batch_size)
    total_solved, total_executed = (0, 0)
    print("start eval")
    for i_batch in range(num_batches):
        ...
        current_batch = dataloader(test_data, args.batch_size, i_batch)
        if current_batch is None:
            print("No more data available.")
            break
